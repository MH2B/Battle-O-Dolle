...
public class MatchMaker : NetworkBehaviour
{
    ...
    public bool HostARoom(bool _isPublic , GameObject _player, string _matchID ,out int _playerIndex , int maxPlayerSize)
	{
        ...
        if (!matchIDs.Contains(_matchID))
        {
            ...
            instance.matchPlayerSize = maxPlayerSize;
            instance.isInBlueTeam = true;
            _playerIndex = 1;
            instance.playerIndex = _playerIndex;
            print("test :" + instance.playerIndex);
            return true;
        }
        else
        {
            ...
            for (int i = 0; i < matches.Count; i++)
            {
                if (matches[i].matchID == _matchID)
                {
                    if (!matches[i].inMatch && !matches[i].isMatchFull)
                    {
                        ...
                        {
                            int teamCount = matches[i].blueTeamPlayers.Count;
                            matches[i].blueTeamPlayers.Add(_player);
                            instance.isInBlueTeam = true;
                            if(teamCount != 0)
							{
                                playerIndex = matches[i].blueTeamPlayers[teamCount - 1].GetComponent<MirrorPlayer>().playerIndex + 1;
                            }
							else
							{
                                playerIndex = 1;
                        ...
                        {
                            int teamCount = matches[i].redTeamPlayers.Count;
                            matches[i].redTeamPlayers.Add(_player);
                            instance.isInBlueTeam = false;
                            if (teamCount != 0)
                            {
                                playerIndex = matches[i].redTeamPlayers[teamCount - 1].GetComponent<MirrorPlayer>().playerIndex + 1;
                            }
                            else
                            {
                                playerIndex = 1;
                        ...
                        instance.matchID = _matchID;
                        instance.matchPlayerSize = matches[i].maxPlayerSize;
    ...
    public int GetThePlayerIndex(string _matchID , bool blueTeam , GameObject _player)
	{
        int index = 0;
        print(matches.Count);
        for (int i = 0; i < matches.Count; i++)
        {
            print("testing id :" + _matchID + ",,,," + matches[i].matchID);
            if (matches[i].matchID == _matchID)
            {
                print("found");
				if (blueTeam)
				{
                    index = matches[i].blueTeamPlayers.IndexOf(_player);
                    print("---" + index);
				}
				else
				{
                    index = matches[i].redTeamPlayers.IndexOf(_player);
                    print("---" + index);
                }
                break;
            }
